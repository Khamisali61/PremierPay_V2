 ...
 * 作者:  wangweicheng
 */
public class TopLocalManage implements ITopUsdk {

    private static final String TAG =  TopLocalManage.class.getSimpleName();
    ...
    private static TopLocalManage mService; // 单例

    private static final String version = "V1.0.10";
    private static Context mContext;
    private static AidlDeviceService mDeviceService;
    private TopUsdkManage.InitListener mListener;

    public TopLocalManage() {
        Log.e(TAG,"TopLocalManage version = " + version);
    }

    public static ITopUsdk getInstance() {
        if (mService == null) {
            synchronized (TopLocalManage.class) {
                if (mService == null) {
                    mService = new TopLocalManage();
                }
            }
        }
        return mService;
    ...
    public AidlMdbSerialport getMDBSerial(int port) {
        try {
            getDeviceService();
            if (mDeviceService != null) {
                return AidlMdbSerialport.Stub.asInterface(mDeviceService.getMdbSerialPort(port));
            }
        } catch (RemoteException e) {
            e.printStackTrace();
        }
