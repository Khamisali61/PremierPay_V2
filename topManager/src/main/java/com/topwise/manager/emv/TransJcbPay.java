...
public class TransJcbPay extends AClssKernelBaseTrans {
    ...
    private AidlJcb jcbPay = TopUsdkManage.getInstance().getJcbPay();
    ...
    public EmvOutCome StartKernelTransProc() {
        try {
            ...
            //如果不设置则内核默认使用“0840”，该值所有非接均未设置。
            setTLV(0x5F2A, new byte[]{0x03, (byte) 0x92});

            //UL L3 JCB测试，非接终端能力配置。
            setTLV(0x9F53, new byte[]{0x73, (byte) 0x80, 0x00});

            //UL L3 JCB测试，支持异常文件处理。
            setTLV(0xDF8134, new byte[]{0x33, 0x00});
            ...
            String cardNo = null;
            if (aucTrack2 != null) {
                cardNo =  getPan(BytesUtil.bytes2HexString(aucTrack2).split("F")[0]);
            ...
            // Start transaction
            //UL L3 JCB案例测试要求比对异常文件列表
            if ("3569990012312327".equals(cardNo)) {
                Log.d(TAG, "StartKernelTransProc: 01");
                nRet = jcbPay.startTrans(0x01);
            } else {
                Log.d(TAG, "StartKernelTransProc: 02");
