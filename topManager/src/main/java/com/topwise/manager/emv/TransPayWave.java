        ...
        try {
            ...
            if (nRet != EmvErrorCode.CLSS_OK) {
                if (nRet == EmvErrorCode.CLSS_RESELECT_APP) {
                    ...
                    }else if(nRet == EmvErrorCode.CLSS_NO_APP){
                        return new EmvOutCome(nRet, ETransStatus.OFFLINE_DECLINED, ETransStep.CLSS_KERNEL_GPO_PROC);
                    }
                    else {
...
//
//                                    if ((aucTTQ.length != 0 && (aucTTQ[2] & 0x80) == 0x00) || (aucCTQ.length != 0 && (aucCTQ[1] & 0x40) == 0x00)) {
//                                        return new EmvOutCome(EmvErrorCode.CLSS_OK, ETransStatus.ONLINE_APPROVE, ETransStep.CLSS_KERNEL_TRANS_COMPLETE);
//                                    }
//
//                                    // Second search card
