...
import com.topwise.manager.emv.impl.EContactCallback;
...
public class ContactEmvProcess extends ABaseTransProcess {
    ...
    public EmvOutCome StartTransProcess() {
        ...
        try {
            ...
            //todo 2025/09/30 disabled for Contactless refund
...
//            }
            ...
            if (emvRest == EmvErrorCode.EMV_OK) {
                ...
                AppLog.d(TAG, "isbSup2GAC : " + !emvTransData.isbSup2GAC());
...
//                    return new EmvOutCome(EmvErrorCode.EMV_FALLBACK, ETransStep.EMV_CANDI_DATA_BUILD);
                    return new EmvOutCome(EmvErrorCode.EMV_TRY_OTHER_INTERFACE, ETransStep.EMV_CANDI_DATA_BUILD);
                } else {
    ...
    private int setTransDataFromAid() {
        ...
        try {
            ...
            //9F41需要以十六进制传入(Premier bank客户要求以纯数字传入)
//            String tag9f41 = String.format("%08x",emvTransData.getTransNo());
            ...
            // Terminal MCC
            String mcc = emvTransData.getMcc();
            AppLog.d(TAG, "emvProcess getMcc = " + mcc);
            emvL2.EMV_SetTLVData(0x9F15, BytesUtil.hexString2Bytes(mcc));
            ...
            if (eaidParam.isbThresholdFlg()) {
                ...
                emvTerminalInfo.setUnThresholdValue((int)convert.strToLong(eaidParam.getAucThreshold(), IConvert.EPaddingPosition.PADDING_RIGHT));
            }
