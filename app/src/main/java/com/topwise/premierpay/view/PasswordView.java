...
public class PasswordView extends LinearLayout {
    ...
    private int mBox_setNumber =6;//数量
    private int mBox_setSize =40;//大小
    private int mBox_setMargin =5;//盒子之间的间距
    ...
    public void  setTextValue(String strings) {
        mBox_nextInputIndex =strings.length();
        String[] mCodeArray =new String[mBox_setNumber];
        for (int i = 0; i<mBox_setNumber; i++) {
            if (i<=strings.length()-1) {
                mCodeArray[i] = strings.substring(i, i + 1);
            } else {
                mCodeArray[i]="";
            }
        }
        this.mCodeArray =mCodeArray;
        postInvalidate();
    }
