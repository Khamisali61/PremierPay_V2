 ...
 * 作者:  wangweicheng
 */
public class TransAuthCmd extends BaseTrans {
    private String title = "";
    private TransData origTransdata;
    public TransAuthCmd(Context context, Handler handler, TransEndListener transListener) {
        super(context, handler, ETransType.TRANS_PRE_AUTH_CMP, transListener);
        title = ETransType.TRANS_PRE_AUTH_CMP.getTransName().toUpperCase();
    }
        ...
        ActionInputData actionInputData = new ActionInputData(new AAction.ActionStartListener() {
            ...
            public void onStart(AAction action) {
                ((ActionInputData)action).setParam(getCurrentContext(),title)
                        .setInputLine1("Original Invoice Number", InputType.TYPE_CLASS_NUMBER, 9, 0, false);
            }
        },handler);
